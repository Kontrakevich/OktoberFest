...
window.addEventListener("load",()=>{fitSalute();
if(document.fonts&&document.fonts.ready)document.fonts.ready.then(fitAddress);
fitAddress();fitAddress();armAutoScroll()});
window.addEventListener("resize",()=>{fitSalute();fitAddress()});
window.addEventListener("orientationchange",()=>setTimeout(()=>{fitSalute();fitAddress()},250));
