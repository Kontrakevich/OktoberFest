...
function autoScroll(){
  ...
  // Phase A: video scrubs at real-time speed.
  const videoRemaining=Math.max(0,videoEndP-startP);
  const videoMs=(videoRemaining/(videoEndP/Math.max(.001,duration)))*1000;

  // Phase B begins before Phase A visually completes because HERO2/fade are already active.
  // The physical scroll continues with no stop into the content landing.
  const videoEndY=scene.offsetTop+travel*videoEndP;
  const tailDistance=Math.max(0,finalY-videoEndY);
  const pxPerMs=Math.max(.45,(videoEndY-startY)/Math.max(1,videoMs));
  const tailMs=Math.max(700,tailDistance/pxPerMs);
  const totalMs=Math.max(250,videoMs+tailMs);
  ...
  function step(now){
    ...
    if(elapsed<=videoMs){
      ...
      const t=clamp((elapsed-videoMs)/Math.max(1,tailMs));
...
function render(){
  ...
  const h2base=ease(clamp((p-.34)/.28));
  ...
  if(hero2){
    ...
    hero2.style.transform='translateY('+(42*(1-h2))+'px)';
  }
  ...
  const fade=easeIO(clamp((p-.56)/.18));
  if(shade)shade.style.opacity=String(.92*fade);

  // Start immediately at video completion; no intermediate stop.
  if(p>=.66 && !tailTriggered){
    tailTriggered=true;
    if(!autoScrollRaf)autoTail();
  }
  if(p<.60)tailTriggered=false;
