...
function render(){
  ...
  const heroExit=easeOut(clamp((p-VIDEO_END)/.08));
  if(hero1){
    hero1.style.opacity=String(1-heroExit);
    hero1.style.transform='translate3d(0,'+(-60*heroExit)+'px,0)';
  }
  ...
  const hero2In=easeOut(clamp((p-VIDEO_END)/.12));
  const hero2Opacity=hero2In;

  if(hero2){
    hero2.style.opacity=String(hero2Opacity);
    hero2.style.transform='translate3d(0,'+(54*(1-hero2In))+'px,0)';
  }

  if(bottomFade){
    bottomFade.style.transform='none';
    bottomFade.style.opacity=String(hero2In);
  }

  // Same darkening as root.
  const d=easeInOut(clamp((p-VIDEO_END)/(1-VIDEO_END)));
