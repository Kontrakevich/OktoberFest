...
function autoScroll(){
  ...
  // Scroll velocity matches the video's real-time playback speed:
  // video duration spans 78% of the hero scroll range.
  const progressPerSecond=.78/Math.max(.001,duration);
  const remainingProgress=Math.max(0,endP-startP);
  const totalMs=Math.max(250,(remainingProgress/progressPerSecond)*1000);
  const started=performance.now();

  function step(now){
    const t=clamp((now-started)/totalMs);
    const p=startP+(endP-startP)*t;
    window.scrollTo(0,scene.offsetTop+travel*p);
...
function render(){
  ...
  const exit=clamp(p/.14),e=ease(exit);
  ...
  const h2base=ease(clamp((p-.56)/.18));
  const tailT=clamp((p-tailStartP)/(tailEndP-tailStartP));
  const h2=Math.max(h2base,tailT);

  if(hero2){
    hero2.style.opacity=String(h2);

    const cs=getComputedStyle(hero2);
    const topPx=parseFloat(cs.top)||0;
    const bottomPx=Math.max(42,innerHeight*.07);
    const lowerY=Math.max(0,innerHeight-bottomPx-hero2.offsetHeight-topPx);

    // v24 behavior until video end; from .78 to .94 HERO2 rises to the top.
    const rise=easeIO(clamp((p-.78)/.16));
    const appearOffset=42*(1-h2);
    hero2.style.transform='translateY('+(lowerY*(1-rise)+appearOffset)+'px)';
  }
  ...
  const fade=easeIO(clamp((p-.78)/.07));
  if(shade)shade.style.opacity=String(.92*fade);

  // Start immediately at video completion; no intermediate stop.
  if(p>=tailStartP && !tailTriggered && !autoScrollRaf){
    tailTriggered=true;
    autoTail();
  }
  if(p<.74)tailTriggered=false;
...
addEventListener("touchstart",e=>{if(!e.target.closest("#hint"))cancelAutoScroll()},{passive:true});addEventListener("resize",fitSalute);addEventListener("orientationchange",()=>setTimeout(fitSalute,250));fitSalute();
document.querySelectorAll(".salute-line").forEach(capFirst);
capFirst(document.querySelector(".section-title"));
const addressLink=document.querySelector(".address a");
if(addressLink){
  addressLink.innerHTML='<span class="address-line">МОСКВА</span><span class="address-line">Ткацкая ул. 5, ст. 7</span>';
}
requestAnimationFrame(render)})();