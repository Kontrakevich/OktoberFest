...
function render(){
  ...
  if(hero1){
    hero1.style.opacity=String(1-heroDim*.82);
    hero1.style.filter='brightness('+(1-heroDim*.62)+')';
    hero1.style.transform='none';
  }

  if(hint){
    hint.style.transform='translate3d(-50%,0,0)';
    hint.style.opacity=String(1-heroDim*.82);
    hint.style.filter='brightness('+(1-heroDim*.62)+')';
  }

  if(bottomFade){
    bottomFade.style.transform='none';
    bottomFade.style.opacity=String(heroDim);
  }
  ...
  if(p>=VIDEO_END && !handoffTriggered){
    ...
    if(firstSection){
      ...
      function handoffStep(now){
        ...
        if(t<1){
          handoffRaf=requestAnimationFrame(handoffStep);
        }else{
          handoffRaf=null;
        }
      }
