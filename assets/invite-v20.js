...
function autoTail(){
  ...
  const targetY=Math.max(0,firstSection.offsetTop-Math.round(innerHeight*.10));
...
function render(){
  ...
  const h2in=ease(clamp((p-.56)/.18)),h2=h2in;
  if(hero2){hero2.style.opacity=String(h2);hero2.style.transform='translateY('+(32*(1-h2in))+'px)'}
  ...
  // As soon as the video reaches its end, darken the whole frame and continue to the HERO2 anchor.
