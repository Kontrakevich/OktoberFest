...
function fitScreen2(){
  ...
  requestAnimationFrame(()=>{
    ...
    const available=screen.clientHeight-(parseFloat(cs.paddingTop)||0)-(parseFloat(cs.paddingBottom)||0);
    const natural=shell.scrollHeight;
    if(natural>available&&natural>0){
      const scale=Math.max(.78,available/natural);
